  ...
  return (
    <>
      <header className={`fixed top-0 left-0 w-full z-[100] transition-all duration-500 ${isScrolled ? "bg-white/95 backdrop-blur-md shadow-md py-3" : "bg-transparent py-5"}`}>
        <div 
          className={`absolute inset-0 -z-10 transition-opacity duration-500 pointer-events-none ${isScrolled ? "opacity-0" : "opacity-100"}`}
          style={{ background: "linear-gradient(to bottom, rgba(0,0,0,0.6) 0%, rgba(0,0,0,0.3) 50%, rgba(0,0,0,0) 100%)" }}
        />
        ...
        <div className="container mx-auto px-4 flex justify-between items-center relative z-10">
          ...
          <nav className="hidden lg:flex items-center space-x-10">
            {["home", "about"].map((item) => (
              <Link key={item} href={`/${locale}/${item === "home" ? "" : item}`} className={`text-[12px] uppercase font-serif font-bold tracking-[0.08em] transition hover:text-[#d48a1f] ${textColor}`}>{t(item)}</Link>
            ))}
            ...
            {["news", "contact"].map((item) => (
              <Link key={item} href={`/${locale}/${item}`} className={`text-[12px] uppercase font-serif font-bold tracking-[0.08em] transition hover:text-[#d48a1f] ${textColor}`}>{t(item)}</Link>
            ))}
          </nav>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", damping: 30, stiffness: 300 }} className="fixed inset-0 bg-white z-[99999] lg:hidden flex flex-col font-serif">
            <div className="flex justify-between items-center p-6 bg-white flex-shrink-0">
              <span className="text-xl font-black tracking-tighter text-[#d48a1f]">LUMA SPA</span>
              <button onClick={() => setIsOpen(false)} className="p-2 text-stone-900"><X size={32} /></button>
            </div>

            {/* PHẦN ĐỔI NGÔN NGỮ MOBILE - DƯỚI LOGO */}
            <div className="px-6 pb-4 flex flex-wrap gap-2 border-b border-stone-100 bg-white">
              {languages.map((lang) => (
                <button
                  key={lang.code}
                  onClick={() => changeLanguage(lang.code.toLowerCase())}
                  className={`flex items-center gap-2 text-[13px] font-serif font-bold tracking-widest transition-all px-3 py-1.5 rounded-full border ${
            ...
            <nav className="flex-grow overflow-y-auto p-6 bg-white">
                  ...
                  return (
                    <div key={key} className="flex flex-col border-b border-stone-50 last:border-0">
                      <div className="flex justify-between items-center">
                        <Link href={`/${locale}/${key === "home" ? "" : key}`} onClick={() => !isServices && setIsOpen(false)} className="flex-grow py-5 text-xl font-bold text-stone-900 uppercase tracking-wide">{t(key)}</Link>
                        {isServices && (
                          <button onClick={() => setShowMobileServices(!showMobileServices)} className="p-5 text-[#d48a1f]">
                      ...
                      <AnimatePresence>
                        {isServices && showMobileServices && (
                          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="bg-stone-50 px-4 flex flex-col pb-4 overflow-hidden rounded-xl">
                            {SERVICES_LIST.map((s) => (
              ...
              <div className="mt-12 pt-8 border-t border-stone-100 pb-10">
